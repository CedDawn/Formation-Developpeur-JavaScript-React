...
tagsInputArray.forEach((input) => {
    input.addEventListener("input", (event) => {
        ...
        if (event.target.value.length >= 3) {
            console.log("au dessus de 3")
            const tags = sessionStorage.getItem('tags');
            const filteredRecipes = filterRecipes(recipes, globalInput.value, JSON.parse(tags));
            tagsResearch(event.target.value, data, filteredRecipes)
        } else {
            console.log("en dessous de 3")
            const tags = sessionStorage.getItem('tags');
            const filteredRecipes = filterRecipes(recipes, globalInput.value, JSON.parse(tags));
...
async function tagsResearch(research, tagType, recipes) {
    ...
    console.log(tags)
    console.log(result)
    const displayFunction = tagType === 'ingredients' ? displayIngredients : tagType === 'appareils' ? displayAppareils : displayUstensiles;
